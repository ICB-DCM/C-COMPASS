                        ...
                        if samplename + '_x' in data_new.columns:
                            for element in list(data_new.index):
                                ...
                                if pd.isnull(data_new[samplename + '_y'][element]):
                                    data_new[samplename + '_y'][element] = data_new[samplename + '_y'][element]
                        data_new = data_new.T.drop_duplicates().T
                        data_new.rename({samplename + '_x' : samplename}, axis = 1, inplace = True)
                    else:
                        data_new = data_new.rename(columns = {samplename: str(sample[1]) + '_Rep.' + str(replicate)})
                        replicate +=1
        
        
        if data_new.applymap(lambda x: ',' in str(x)).any().any():
            data_new = data_new.applymap(lambda x: str(x).replace(',', '.') if isinstance(x, str) else x)
        ...
        for rep_own in data[condition].columns.tolist():
            for rep_other in data[condition].columns.tolist():
                if not rep_own == rep_other:
                    corrs.append(pearsonr(data[condition][rep_own].tolist(), data[condition][rep_other].tolist())[0])
    ...
    for condition in data:
        ...
        if mode == 'normal':
            for sample in data[condition]:
                ...
                mean_imp = mean - s*std
                sigma = std*w
                data[condition][sample] = data[condition][sample].apply(lambda x: np.random.normal(mean_imp, sigma, 1)[0] if math.isnan(x) else x)
        elif mode == 'constant':
            for sample in data[condition]:
                data[condition][sample] = data[condition][sample].apply(lambda x: 0 if math.isnan(x) else x)
    return data
...
def normalize_data (data, window):
    ...
    for condition in data:
        for replicate in data[condition]:
            ...
            data[condition][replicate] = data[condition][replicate].apply(lambda x: (x-q2) / (q3-q2) if x-q2 >= 0 else (x-q2) / (q2-q1))
    return data
...
def TPP_exec (window, tp_data, tp_tables, tp_preparams, tp_identifiers, tp_intermediate, tp_info, tp_icorr, tp_indata, tp_conditions):
    ...
    layout_TPP = [
        [sg.Column([
            [sg.ProgressBar(60, orientation = 'h', size = (38,25), key = '--progress--')],
            [sg.Text('-ready-', font = ('Arial', 9), size = (60,2), pad = (1,1), key = '--status1--')],
            [sg.Text('for run', font = ('Arial', 9), size = (60,2), pad = (1,1), key = '--status2--')],
            ], size = (420,100)),
        sg.Column([
            [sg.Button('Start', size = (15,1), key = '--start--', disabled = False, enable_events = True)],
            [sg.Button('Cancel', size = (15,1), key = '--cancel--', disabled = False, enable_events = True, button_color = 'black')]
            ], size = (150,70))]
              ]
    window_TPP = sg.Window('Processing...', layout_TPP, size = (600,110))
    ...
    while True:
        ...
        if event_TPP == '--start--':
            ...
            if not is_con:
                ...
                tp_data, tp_info, tp_conditions = create_dataset(tp_indata, tp_tables, tp_identifiers, conditions, window_TPP)
            ...
            window_TPP.close()
            event_TPP, values_TPP = window.read(timeout = 50)
            print('done!')
            break
