...
def plot_compressed(data, classes, name):
    ...
    for i in range(len(classes)):
        ...
        if organelle == 'none':
            current_color = 'grey'
        scatter = ax.scatter(data_class[0], data_class[1],
                         s = 0.2,
                         color = current_color,
                         label = organelle)
...
def create_wxyz (fract_conditions, fract_marker, fract_marker_up, fract_test, fract_full, fract_full_up, fract_mixed_up):
    ...
    for condition in conditions:
        ...
        x_train_mixed_up = fract_mixed_up[condition].drop(columns = classes).to_numpy(dtype = float)
        ...
        Z_train_up = pd.get_dummies(fract_marker_up[condition]['class']).to_numpy(dtype = float)
...
def SOP_exec (learning_wxyz, fract_conditions, fract_marker, fract_test):
    ...
    for condition in conditions:
        ...
        svm_y_train_prob = list(map(lambda x: max(x), list(clf.predict_proba(svm_x_train))))
        svm_y_test_prob = list(map(lambda x: max(x), list(clf.predict_proba(svm_x_test))))
        
        
        svm_confusion = pd.DataFrame(confusion_matrix(svm_Y_train, svm_y_train, labels = list(clf.classes_)), index = clf.classes_, columns = clf.classes_)
        ...
        svm_metrics[condition] = {'confusion' : svm_confusion,
...
def plot_umaps (learning_wxyz, fract_marker, fract_marker_up, fract_test, fract_full, fract_full_up):
    umaps = {}
    ...
    for condition in learning_wxyz:
        ...
        full_up_umap_df['class'] = list(fract_full_up[condition].fillna('none')['class'])
