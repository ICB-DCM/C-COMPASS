...
def class_reset (results, comparison):
    for condition in results:
        ...
        results[condition]['class_abundance'] = {}
        results[condition]['metrics'].drop(['TPA',
                                            'CA_relevant'], axis = 1, inplace = True)
        
        classnames = results[condition]['classnames']
        for classname in classnames:
            results[condition]['metrics'].drop(['nCClist_'+classname,
                                                'nCC_' + classname,
                                                'CPA_'+classname,
                                                'CPA_log_'+classname,
                                                'CPA_imp_'+classname,
                                                'nCPA_'+classname,
                                                'nCPA_log_'+classname,
                                                'nCPA_imp_'+classname], axis = 1, inplace = True)
    for comb in comparison:
        comparison[comb]['metrics'].drop(['nRLS',
                                          'P(t)_nRLS',
                                          'P(u)_nRLS'], axis = 1, inplace = True)
        comparison[comb] = {k: v for k, v in comparison[comb].items() if k not in ['nRLS_results',
                                                                                   'nRLS_null']}
        classnames = results[comb[0]]['classnames']
        for classname in classnames:
            comparison[comb]['metrics'].drop(['nRL_'+classname,
                                              'CFC_'+classname,
                                              'nCFC_'+classname], axis = 1, inplace = True)
