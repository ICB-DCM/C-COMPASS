...
def create_condition_dataset (data_init, tables_init, conditions, idents):      # creates dictionary containing data sorted by condition
    dataset = {}
    data = {}
    identifier = []
...
def scale_data (dataset):                                                       # scales a dataset from 0 to 1
    scaler = MinMaxScaler()
    dataset_scaled = pd.DataFrame(scaler.fit_transform(dataset.T).T, columns = dataset.columns)
...
def create_repdata (condata):                                                   # creates dictionary containing data sorted by replicate
    ...
    for rep in replist:
        ...
        for sample in condata.columns:
            ...
            if suffix == rep:
                data = pd.merge(data, condata[sample], left_index = True, right_index = True)
        repdata[rep] = data
    return(repdata)
...
def calculate_innercorr (condata, fracts_corr, protlist_con, condition):
    ...
    for replicate in condata:
        ...
        icorr = pd.merge(icorr, correls[replicate], left_index = True, right_index = True, how = 'outer')
    return icorr
...
def create_dataset (data_all, tables_all, identifier, conditions, values_NPC):
    for i in ['', '[IDENTIFIER]']:                                              # create dataset to work with
        if i in conditions:
            conditions.remove(i) 
    data_con_abs = create_condition_dataset(data_all, tables_all, conditions, identifier)
    data_keep = {}
...
def filter_missing (data_con, stats, values_NPC):
    ...
    for condition in data_con:                                          # filter by Missing Values / Valid Values
        ...
        for replicate in condata:
            ...
            repdata.replace(np.nan, 0., inplace = True)
            count_after = len(repdata)
            stats['filtered']['by ValidValues'][condition + '_' + replicate] = count_after - count_before
...
def filter_count (data_con, mincount, stats):                        # filter by count over replicates
    ...
    for condition in data_con:
        ...
        for replicate in condata:
            ...
            count_after = len(repdata)
            stats['filtered']['by count'][condition + '_' + replicate] = count_after - count_before
            condata[replicate] = repdata
...
def calculate_icorr (data_con, fracts_corr, protlist_con):
    icorr = {}
    for condition in data_con:
        icorr_sub = calculate_innercorr(data_con[condition], fracts_corr, protlist_con, condition)
            ...
            for replicate in condata:
                ...
                for sample in repdata:
                    if sample[:sample.find('_')] == prefix:
                        fract_vals = pd.merge(fract_vals, repdata[sample], left_index = True, right_index = True, how = 'outer')
                        fract_std = pd.merge(fract_std, repdata[sample], left_index = True, right_index = True, how = 'outer')
...
def create_concat (data_con, scale):
    ...
    for condition in data_con:
        ...
        for replicate in condata:
            ...
            con_vals = pd.merge(con_vals, repdata, left_index = True, right_index = True, how = 'outer')
            con_vals.rename(renamedict, axis = 'columns', inplace = True)
        if scale:
            con_vals = scale_data(con_vals)
        data_con_concat[condition] = con_vals.fillna(0.)
...
def remove_zeros (data_con, stats, params):
    for condition in data_con:
        ...
        stats['filtered']['by baseline profiles'][condition + '_' + params['mode']] = count_after - count_before
        data_con[condition] = condata
    return data_con
    ...
    for condition in data_con:
        for con in data_con:
            if not con == condition:
                ...
                data_own = data_con[condition].fillna(0.)
                data_other = data_con[con].fillna(0.)
