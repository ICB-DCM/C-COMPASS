...
def PPMS_exec_fract(params_old):
    ...
    while True:
        ...
        if event_PPMS == "--class_scale1--":
            window_PPMS["--class_scale1_how--"].Update(
                disabled=not values_PPMS["--class_scale1--"]
            )
        if event_PPMS == "--vis_scale1--":
            window_PPMS["--vis_scale1_how--"].Update(
                disabled=not values_PPMS["--vis_scale1--"]
            )
        ...
        if event_PPMS == "--class_scale2--":
            window_PPMS["--class_scale2_how--"].Update(
                disabled=not values_PPMS["--class_scale2--"]
            )
        if event_PPMS == "--vis_scale2--":
            window_PPMS["--vis_scale2_how--"].Update(
                disabled=not values_PPMS["--vis_scale2--"]
            )

        if event_PPMS == "--glob_missing--":
            window_PPMS["--glob_minval--"].Update(
                disabled=not values_PPMS["--glob_missing--"]
            )
        if event_PPMS == "--glob_minrep--":
            window_PPMS["--glob_mincount--"].Update(
                disabled=not values_PPMS["--glob_minrep--"]
            )
