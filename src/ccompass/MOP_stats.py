...
def stats_exec3 (mode, learning_xyz, NN_params, fract_data, tp_data, fract_marker, fract_conditions, fract_std):
    ...
    for condition in conditions:
        ...
        for subcon in subcons:
            marker_df = learning_xyz[subcon]['W_train_df'][~learning_xyz[subcon]['W_train_df'].index.duplicated(keep='first')]
            results[condition]['metrics']['marker'] = results[condition]['metrics']['marker'].fillna(marker_df)
        
        ## add SVM results:
        results[condition]['SVM'] = {}
        results[condition]['SVM']['winner_combined'] = pd.DataFrame(index = results[condition]['metrics'].index)
        results[condition]['SVM']['prob_combined'] = pd.DataFrame(index = results[condition]['metrics'].index)
        for subcon in subcons:
            print(subcon)
            learning_xyz[subcon]['w_full_combined'] = pd.DataFrame(index = learning_xyz[subcon]['x_full_df'].index)
            learning_xyz[subcon]['w_full_prob_combined'] = pd.DataFrame(index = learning_xyz[subcon]['x_full_df'].index)
            
            for roundn in learning_xyz[subcon]['w_full_prob_df']:
                learning_xyz[subcon]['w_full_combined'] = pd.merge(learning_xyz[subcon]['w_full_combined'], learning_xyz[subcon]['w_full_prob_df'][roundn]['SVM_winner'], left_index = True, right_index = True, how = 'left')
                learning_xyz[subcon]['w_full_combined'] = learning_xyz[subcon]['w_full_combined'].loc[~learning_xyz[subcon]['w_full_combined'].index.duplicated(keep='first')]
                learning_xyz[subcon]['w_full_combined'].rename(columns = {'SVM_winner' : roundn+'_SVM_winner'}, inplace = True)
                learning_xyz[subcon]['w_full_prob_combined'] = pd.merge(learning_xyz[subcon]['w_full_prob_combined'], learning_xyz[subcon]['w_full_prob_df'][roundn]['SVM_prob'], left_index = True, right_index = True, how = 'left')
                learning_xyz[subcon]['w_full_prob_combined'] = learning_xyz[subcon]['w_full_prob_combined'].loc[~learning_xyz[subcon]['w_full_prob_combined'].index.duplicated(keep='first')]
                learning_xyz[subcon]['w_full_prob_combined'].rename(columns = {'SVM_prob' : roundn+'_SVM_prob'}, inplace = True)
            
            SVM_equal = learning_xyz[subcon]['w_full_combined'].apply(lambda row: row.nunique() == 1, axis=1)
            learning_xyz[subcon]['w_full_combined']['SVM_winner'] = np.where(SVM_equal, learning_xyz[subcon]['w_full_combined'].iloc[:, 0], np.nan)
            learning_xyz[subcon]['w_full_prob_combined']['SVM_prob'] = learning_xyz[subcon]['w_full_prob_combined'].mean(axis = 1)
            
            learning_xyz[subcon]['w_full_combined'] = pd.merge(learning_xyz[subcon]['w_full_combined'], learning_xyz[subcon]['w_full_prob_combined'][['SVM_prob']], left_index = True, right_index = True, how = 'left')
            learning_xyz[subcon]['w_full_combined'] = learning_xyz[subcon]['w_full_combined'].loc[~learning_xyz[subcon]['w_full_combined'].index.duplicated(keep='first')]
            learning_xyz[subcon]['w_full_combined'].loc[learning_xyz[subcon]['w_full_combined']['SVM_winner'].isna(), 'SVM_prob'] = np.nan
            
            results[condition]['SVM']['winner_combined'] = pd.merge(results[condition]['SVM']['winner_combined'], learning_xyz[subcon]['w_full_combined']['SVM_winner'], left_index = True, right_index = True, how = 'left')
            results[condition]['SVM']['winner_combined'] = results[condition]['SVM']['winner_combined'].loc[~results[condition]['SVM']['winner_combined'].index.duplicated(keep='first')]
            results[condition]['SVM']['prob_combined'] = pd.merge(results[condition]['SVM']['prob_combined'], learning_xyz[subcon]['w_full_combined']['SVM_prob'], left_index = True, right_index = True, how = 'left')
            results[condition]['SVM']['prob_combined'] = results[condition]['SVM']['prob_combined'].loc[~results[condition]['SVM']['prob_combined'].index.duplicated(keep='first')]
        
        SVM_equal = results[condition]['SVM']['winner_combined'].apply(lambda row: row.nunique() == 1, axis=1)
        #SVM_equal = 
        SVM_major = results[condition]['SVM']['winner_combined'].apply(most_frequent_or_nan, axis=1)
        SVM_major.name = 'SVM_subwinner'
        results[condition]['metrics']['SVM_winner'] = np.where(SVM_equal, results[condition]['SVM']['winner_combined'].iloc[:, 0], np.nan)
        results[condition]['metrics'] = pd.merge(results[condition]['metrics'], SVM_major, left_index = True, right_index = True, how = 'left')
        prob_means = results[condition]['SVM']['prob_combined'].mean(axis=1)
        results[condition]['metrics']['SVM_prob'] = np.nan
        results[condition]['metrics'].loc[results[condition]['metrics']['SVM_winner'].notna(), 'SVM_prob'] = prob_means
        ...
        for subcon in subcons:
            ...
            learning_xyz[subcon]['z_full_mean_df'] = pd.DataFrame(learning_xyz[subcon]['z_full_mean'], index = learning_xyz[subcon]['x_full_df'].index, columns = learning_xyz[subcon]['Z_train_df'].columns)
        ...
        for classname in classnames:
            ...
            for subcon in subcons:
                CC_list = pd.merge(CC_list, learning_xyz[subcon]['z_full_mean_df'][classname], left_index = True, right_index = True, how = 'left')
            
            CC_list['CClist_'+classname] = CC_list.apply(lambda row: row.tolist(), axis=1)
            
            results[condition]['metrics'] = pd.merge(results[condition]['metrics'], CC_list['CClist_'+classname], left_index = True, right_index = True, how = 'left')
            results[condition]['metrics'] = results[condition]['metrics'].loc[~results[condition]['metrics'].index.duplicated(keep='first')]
        ...
        for classname in classnames:
            ...
            results[condition]['metrics']['CC_'+classname] = results[condition]['metrics']['CClist_'+classname].apply(lambda x: np.nanmean(x) if x else np.nan)
        cc_cols = [col for col in results[condition]['metrics'].columns if col.startswith('CC_')]
        ...
        results[condition]['metrics'][cc_cols] = results[condition]['metrics'][cc_cols].div(cc_sums, axis=0)
        ...
        ## add NN_winner:
        cc_columns = results[condition]['metrics'][[col for col in results[condition]['metrics'].columns if col.startswith('CC_')]]
        ...
        for class_act in classnames:
            nonmarker_z = results[condition]['metrics'].loc[
                ...
                (results[condition]['metrics']['marker'].isna() == False)][['CC_'+class_act]]
            thresh = np.percentile(nonmarker_z['CC_'+class_act].tolist(), NN_params['reliability'])
            results[condition]['metrics']['fCC_'+class_act] = results[condition]['metrics']['CC_'+class_act]
            results[condition]['metrics'].loc[results[condition]['metrics']['fCC_' + class_act] < thresh, 'fCC_' + class_act] = 0.
        
        fcc_cols = [col for col in results[condition]['metrics'] if col.startswith('fCC_')]
        fcc_sums = results[condition]['metrics'][fcc_cols].sum(axis=1)
        #fcc_sums[fcc_sums == 0] = 1
        results[condition]['metrics'][fcc_cols] = results[condition]['metrics'][fcc_cols].div(fcc_sums, axis = 0)
        
        
        ## add fNN_winner:
        fcc_columns = results[condition]['metrics'][[col for col in results[condition]['metrics'].columns if col.startswith('fCC_')]]
...
def stats_exec2 (learning_xyz, NN_params, tp_data, fract_marker):
    ...
    for condition in conditions:
        print(condition+'...')
        results[condition] = {}
        results[condition]['metrics'] = pd.DataFrame(index = learning_xyz[condition]['x_full_df'].index)
        ...
        for roundn in learning_xyz[condition]['w_full_prob_df']:
            winner_list.append(learning_xyz[condition]['w_full_prob_df'][roundn]['SVM_winner'])
            score_list.append(learning_xyz[condition]['w_full_prob_df'][roundn]['SVM_prob'])
        combined_winner = pd.concat(winner_list, axis = 1)
        results[condition]['metrics']['SVM_winner'] = combined_winner.apply(reduce_to_single_column, axis = 1)
        combined_score = pd.concat(score_list, axis = 1)
        results[condition]['metrics']['SVM_prob'] = combined_score.mean(axis = 1)
        results[condition]['metrics'].loc[results[condition]['metrics']['SVM_winner'].isna(), 'SVM_prob'] = np.nan
        
        # add CA:
        results[condition]['metrics'] = pd.merge(results[condition]['metrics'], fract_marker[condition]['class'], left_index = True, right_index = True, how = 'left').rename(columns = {'class' : 'marker'})
        results[condition]['metrics']['CA_relevant'] = 'no'
        results[condition]['class_abundance'] = {}
        # classnames = []
        for classname in [classname for classname in results[condition]['metrics']['SVM_winner'] if not pd.isnull(classname)]:
            results_class = results[condition]['metrics'][(results[condition]['metrics']['SVM_winner'] == classname) &
            ...
            results[condition]['class_abundance'][classname] = {}
            results[condition]['class_abundance'][classname]['CA'] = np.median(results_class['TPA'])
            results[condition]['class_abundance'][classname]['std'] = np.std(results_class['TPA'])
            results[condition]['class_abundance'][classname]['count'] = len(results_class)
        ...
        stacked_CC = np.array([array / array.sum(axis=1, keepdims=True) for array in stacked_CC])
        ...
        median_CC_df = pd.DataFrame(normalized_median_CC, columns = list(learning_xyz[condition]['z_full_df'].values())[0].columns, index = list(learning_xyz[condition]['z_full_df'].values())[0].index).add_prefix('CC_')
        IQR_CC_df = pd.DataFrame(IQR_CC, columns = list(learning_xyz[condition]['z_full_df'].values())[0].columns, index = list(learning_xyz[condition]['z_full_df'].values())[0].index).add_prefix('dCC_')
        
        results[condition]['metrics'] = pd.merge(results[condition]['metrics'], median_CC_df, left_index = True, right_index = True, how = 'left')
        results[condition]['metrics'] = pd.merge(results[condition]['metrics'], IQR_CC_df, left_index = True, right_index = True, how = 'left')
        ...
        fCC_columns = [col for col in results[condition]['metrics'].columns if col.startswith('fCC_')]
        row_sums = results[condition]['metrics'][fCC_columns].sum(axis=1)
        results[condition]['metrics'][fCC_columns] = results[condition]['metrics'][fCC_columns].div(row_sums, axis=0)
        ...
        list_df = pd.DataFrame(data, index=list(learning_xyz[condition]['z_full_df'].values())[0].index)
        
        results[condition]['metrics'] = pd.merge(results[condition]['metrics'], list_df, left_index = True, right_index = True, how = 'left')
        results[condition]['classnames'] = classnames
...
def compare_lists(list1, list2):
    ...
    return sum(abs(a - b) for a, b in zip(list1, list2) if not pd.isna(a) and not pd.isna(b))

def comp_exec3 (mode, results, learning_xyz):
    conditions = [x for x in results]
    for condition in conditions:
        results[condition]['metrics'] = results[condition]['metrics'][~results[condition]['metrics'].index.duplicated(keep = 'first')]
    ...
    for comb in combinations:
        print(comb)
        classnames = list(set(results[comb[0]]['classnames']) & set(results[comb[1]]['classnames']))
        comparison[comb] = {}
        
        ## prepare data:
        comparison[comb]['intersection_data'] = pd.merge(results[comb[0]]['metrics'], results[comb[1]]['metrics'], left_index = True, right_index = True, how = 'inner')
        ...
        for classname in classnames:
            ...
            if mode == 'deep':
                ...
                pass
        rl_cols = [col for col in comparison[comb]['metrics'].columns if col.startswith('RL_')]
        comparison[comb]['metrics']['RLS'] = comparison[comb]['metrics'][rl_cols].abs().sum(axis=1)
        if mode == 'deep':
        ...
        for classname in classnames:
            comparison[comb]['metrics']['fRL_'+classname] = results[comb[1]]['metrics']['fCC_'+classname] - results[comb[0]]['metrics']['fCC_'+classname]
        frl_cols = [col for col in comparison[comb]['metrics'].columns if col.startswith('fRL_')]
        comparison[comb]['metrics']['fRLS'] = comparison[comb]['metrics'][frl_cols].abs().sum(axis=1)
        ...
        test_df, common_indices, ncc_columns = perform_mann_whitney_t_tests_per_cell(metrics_own, metrics_other, 'CClist_')
        # for classname in classnames:
        ...
        for classname in classnames:
            test_df.rename(columns = {'CClist_'+classname+'_U' : 'U_'+classname,
                                      'CClist_'+classname+'_T' : 'T_'+classname,
                                      'CClist_'+classname+'_D' : 'D_'+classname,
                                      'CClist_'+classname+'_P(U)' : 'P(U)_'+classname,
                                      'CClist_'+classname+'_P(T)' : 'P(T)_'+classname}, inplace = True)
            # calculate DS:
        d_columns = [col for col in test_df.columns if col.startswith('D_')]
        test_df['DS'] = test_df[d_columns].abs().sum(axis=1)
        
            # add statistics to metrics:
        comparison[comb]['metrics'] = pd.merge(comparison[comb]['metrics'], test_df, left_index = True, right_index = True, how = 'left')
        ...
        for ID in common_indices:
            cclists_own = [metrics_own.loc[ID, 'CClist_' + classname] for classname in classnames]
            cclists_other = [metrics_other.loc[ID, 'CClist_' + classname] for classname in classnames]
            ...
            for i in range(len(cclists_own_transposed)):
                for j in range(i+1, len(cclists_own_transposed)):
                    null_result = compare_lists(cclists_own_transposed[i], cclists_own_transposed[j])
                    RLS_null[ID].append(null_result)
            for i in range(len(cclists_other_transposed)):
                for j in range(i+1, len(cclists_other_transposed)):
                    null_result = compare_lists(cclists_other_transposed[i], cclists_other_transposed[j])
                    RLS_null[ID].append(null_result)
        ...
        for index in comparison[comb]['metrics'].index:
            if index in common_indices:
                # Perform the t-test
                stat, p_value = ttest_ind(comparison[comb]['RLS_results'].loc[index], comparison[comb]['RLS_null'].loc[index], nan_policy='omit')
                comparison[comb]['metrics'].loc[index, 'P(t)_RLS'] = p_value
                if is_all_nan(comparison[comb]['RLS_results'].loc[index]) or is_all_nan(comparison[comb]['RLS_null'].loc[index]) or len(set(comparison[comb]['RLS_results'].loc[index])) == 1 or len(set(comparison[comb]['RLS_null'].loc[index])) == 1:
                    comparison[comb]['metrics'].loc[index, 'P(u)_RLS'] = pd.NA
                else:
                    stat_u, p_value_u = stats.mannwhitneyu(comparison[comb]['RLS_results'].loc[index], comparison[comb]['RLS_null'].loc[index], alternative='two-sided')
...
def comp_exec2 (results, learning_xyz):
    ...
    for comb in combinations:
        ...
        for classname in classnames:
            ...
            comparison[comb]['metrics'] = benjamini_hochberg_correction(comparison[comb]['metrics'], col_old, col_new)
        ...
        comparison[comb]['metrics'] = benjamini_hochberg_correction(comparison[comb]['metrics'], 't2_P', 't2_FDR')
...
def hotelling_t2_test(data1, data2, regularization_value=1e-6):
    ...
    mean_diff = mean1 - mean2
    t_squared = n1 * n2 / (n1 + n2) * np.dot(mean_diff.T, np.dot(np.linalg.inv(pooled_cov), mean_diff))
...
def process_and_test(data_own, data_other, classnames):
    ...
    filtered_cols = [col for col in data_own.columns if col.startswith('CClist_') and col.split('_')[1] in classnames]
    ...
    for idx in data_own.index:
        ...
        valid_cols = [col for col in filtered_cols if col in data_own.columns and col in data_other.columns]
        row_df1_data, row_df2_data = [], []

        for col in valid_cols:
            val1, val2 = data_own.at[idx, col], data_other.at[idx, col]
            if isinstance(val1, list) and isinstance(val2, list) and not all(np.isnan(val1)) and not all(np.isnan(val2)):
...
def impute_data (df, colname, newcol):
    ...
    mean = np.mean(df[colname].replace(-np.inf, np.nan))  # Exclude 0s from mean calculation
    std = np.std(df[colname].replace(-np.inf, np.nan))    # Exclude 0s from std calculation
...
def is_all_nan(list_):
    return all(np.isnan(x) for x in list_) if isinstance(list_, list) else np.isnan(list_)
...
def perform_paired_t_tests_per_cell(df1, df2, prefix):
    ...
    result_columns = [f"{col}_T" for col in cc_columns] + [f"{col}_P" for col in cc_columns] + [f"{col}_D" for col in cc_columns]
    results_df = pd.DataFrame(index=common_indices, columns=result_columns)
    ...
    for col in cc_columns:
        for idx in common_indices:
            ...
            if is_all_nan(list_df1) or is_all_nan(list_df2) or len(set(list_df1)) == 1 or len(set(list_df2)) == 1:
                ...
                # Perform Mann-Whitney U test
                u_stat, p_value_u = stats.mannwhitneyu(list_df1, list_df2, alternative='two-sided')

                # Perform t-test
                t_stat, p_value_t = stats.ttest_ind(list_df1, list_df2, equal_var=False, nan_policy='omit')
...
def stats_exec (learning_xyz, tp_data):
    ...
    for condition in conditions:
        ...
        for classname in [classname for classname in results[condition]['metrics']['SVM_winner'] if not pd.isnull(classname)]:
            ...
            results[condition]['metrics'].loc[results_class.index, 'CA_relevant'] = 'yes'
            results[condition]['class_abundance'][classname] = {}
            results[condition]['class_abundance'][classname]['mean'] = np.mean(results_class['TPA'])
            results[condition]['class_abundance'][classname]['std'] = np.std(results_class['TPA'])
            results[condition]['class_abundance'][classname]['count'] = len(results_class)
        ...
        for classname in classnames:
            ...
            results[condition]['metrics']['CC_'+classname] = combined_CC.median(axis = 0)
        ...
        print('calculate pCC...')
        for classname in classnames:
            results[condition]['metrics']['pCC_'+classname] = results[condition]['metrics']['CC_'+classname] / results[condition]['class_abundance'][classname]['mean']
            results[condition]['metrics']['dpCC_'+classname] = abs(1/results[condition]['class_abundance'][classname]['mean']) * results[condition]['metrics']['dCC_'+classname] + abs(-results[condition]['metrics']['CC_'+classname] / (results[condition]['class_abundance'][classname]['mean'])**2) * results[condition]['class_abundance'][classname]['std'] 
        
        print('calculate nCC...')
        nCC_col = [col for col in results[condition]['metrics'].columns if col.startswith('pCC')]
        for col in nCC_col:
            nCC_col = col.replace('pCC_', 'nCC_')
            results[condition]['metrics'][nCC_col] = results[condition]['metrics'][col]
        nCC_cols = [col for col in results[condition]['metrics'].columns if col.startswith('nCC_')]
        results[condition]['metrics'][nCC_cols] = results[condition]['metrics'][nCC_cols].div(results[condition]['metrics'][nCC_cols].sum(axis = 1), axis = 0)
        for classname in classnames:
            results[condition]['metrics']['CC_factor_'+classname] = results[condition]['metrics']['nCC_'+classname] / results[condition]['metrics']['pCC_'+classname]
            results[condition]['metrics']['dnCC_'+classname] = results[condition]['metrics']['dpCC_'+classname] * results[condition]['metrics']['CC_factor_'+classname]
        
        print('calculate CPA...')
        for classname in classnames:
            results[condition]['metrics']['CPA_'+classname] = results[condition]['metrics']['CC_'+classname] * results[condition]['metrics']['TPA']
            results[condition]['metrics']['dCPA_'+classname] = abs(results[condition]['metrics']['TPA']) * results[condition]['metrics']['dCC_'+classname] + abs(results[condition]['metrics']['CC_'+classname]) * results[condition]['metrics']['dTPA']
        
        
        print('calculate rCPA...')
        for classname in classnames:
            results[condition]['metrics']['rCPA_'+classname] = results[condition]['metrics']['nCC_'+classname] * results[condition]['metrics']['TPA']
            results[condition]['metrics']['drCPA_'+classname] = abs(results[condition]['metrics']['TPA']) * results[condition]['metrics']['dnCC_'+classname] + abs(results[condition]['metrics']['nCC_'+classname]) * results[condition]['metrics']['dTPA']
...
def comp_exec (learning_xyz, results):
    ...
    for comb in combinations:
        results[comb] = {}
        results[comb]['intersection_data'] = pd.merge(learning_xyz[comb[0]]['x_full_df'], learning_xyz[comb[1]]['x_full_df'], left_index = True, right_index = True, how = 'inner')
        results[comb]['comparison'] = pd.DataFrame(index = results[comb]['intersection_data'].index)
        ...
        for classname in classnames:
            results[comb]['comparison']['RL_'+classname] = metrics_other['nCC_'+classname] - metrics_own['nCC_'+classname]
            results[comb]['comparison']['dRL_'+classname] = metrics_other['dnCC_'+classname] + metrics_own['dnCC_'+classname]
        
        RLS_columns = [col for col in results[comb]['comparison'].columns if col.startswith('RL_')]
        dRLS_columns = [col for col in results[comb]['comparison'].columns if col.startswith('dRL_')]
        
        results[comb]['comparison']['RLS'] = results[comb]['comparison'][RLS_columns].abs().sum(axis = 1)
        results[comb]['comparison']['dRLS'] = results[comb]['comparison'][dRLS_columns].abs().sum(axis = 1)
        
        for classname in classnames:
            results[comb]['comparison']['RC_'+classname] = metrics_other['CC_'+classname] - metrics_own['CC_'+classname]
            results[comb]['comparison']['dRC_'+classname] = metrics_other['dCC_'+classname] + metrics_own['dCC_'+classname]
        
        RCS_columns = [col for col in results[comb]['comparison'].columns if col.startswith('RC_')]
        dRCS_columns = [col for col in results[comb]['comparison'].columns if col.startswith('dRC_')]
        
        results[comb]['comparison']['RCS'] = results[comb]['comparison'][RCS_columns].abs().sum(axis = 1)
        results[comb]['comparison']['dRCS'] = results[comb]['comparison'][dRCS_columns].abs().sum(axis = 1)
        
        for classname in classnames:
            results[comb]['comparison']['CPD_'+classname] = metrics_other['rCPA_'+classname] - metrics_own['rCPA_'+classname]
            results[comb]['comparison']['dCPD_'+classname] = metrics_other['drCPA_'+classname] + metrics_own['drCPA_'+classname]
    return results
