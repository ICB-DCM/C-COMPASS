...
def create_dataset (input_data, input_tables, identifiers, conditions, window_FDP, progress):
    for i in ['', '[IDENTIFIER]']:
        if i in conditions:
            conditions.remove(i) 
    dataset = {}
    ...
    for condition in conditions:
        ...
        for path in input_tables:
            for sample in input_tables[path]:
                ...
                if sample[1] == condition:
                    ...
                    data_new = pd.merge(data_new, data, right_index = True, left_index = True, how = 'outer')
                    if condition == '[KEEP]':
                        if samplename + '_x' in data_new.columns:
                            for element in list(data_new.index):
                                if pd.isnull(data_new[samplename + '_x'][element]):
                                    data_new[samplename + '_x'][element] = data_new[samplename + '_y'][element]
                                if pd.isnull(data_new[samplename + '_y'][element]):
                                    data_new[samplename + '_y'][element] = data_new[samplename + '_x'][element]
                        data_new = data_new.T.drop_duplicates().T
                        data_new.rename({samplename + '_x' : samplename}, axis = 1, inplace = True)
                    else:
                        data_new = data_new.rename(columns = {samplename:
                                                          'Fr.' + str(sample[3]) + '_' +
                                                          samplename +
                                                          '_Rep.' + str(sample[2])})
        repdata = {}
        ...
        for rep in replist:
            ...
            for sample in data_new.columns:
                ...
                if suffix == rep:
                    data = pd.merge(data, data_new[sample], left_index = True, right_index = True)
            repdata[rep] = data
...
def pre_post_scaling (data, how, window_FDP, progress):
    if how == 'minmax':
        for condition in data:
            stepsize = (5./len(data))/len(data[condition])
            for replicate in data[condition]:
                ...
                event, values = window_FDP.read(timeout = 50)
                scaler = MinMaxScaler()
                data[condition][replicate] = pd.DataFrame(scaler.fit_transform(data[condition][replicate].T).T, columns = data[condition][replicate].columns).set_index(data[condition][replicate].index)
    if how == 'area':
        for condition in data:
            stepsize = (5./len(data))/len(data[condition])
            for replicate in data[condition]:
                ...
                window_FDP['--status2--'].Update(' '.join([condition, replicate]))
                event, values = window_FDP.read(timeout = 50)
                data[condition][replicate] = data[condition][replicate].div(data[condition][replicate].sum(axis = 1), axis = 0)
    return data, progress


def filter_missing (data, mincount, window_FDP, progress):
    for condition in data:
        stepsize = (5./len(data))/len(data[condition])
        for replicate in data[condition]:
            progress = progress + stepsize
            ...
            data[condition][replicate].replace(np.nan, 0., inplace = True)
    return data, progress
...
def list_samples (data, window_FDP, progress):
    ...
    for condition in data:
        ...
        fractions = []
        stepsize = (10./len(data))/len(data[condition])
        for replicate in data[condition]:
            progress = progress + stepsize
            window_FDP['--progress--'].Update(progress)
            window_FDP['--status2--'].Update(' '.join([condition, replicate]))
            ...
            for sample in list(data[condition][replicate].columns):
                ...
                if fractnumber not in fractions:
                    ...
                    fracts_count[condition][fractnumber] = fracts_count[condition][fractnumber] +1
        fractions = sorted(fractions)
        fracts_con[condition] = fractions
        fracts_corr[condition] = ['Fr.'+str(k) for k, v in fracts_count[condition].items() if v == max(fracts_count[condition].values())]
...
def calculate_icorr (data, fracts_corr, protlist_con, window_FDP):
    ...
    for condition in data:
        ...
        for replicate in data[condition]:
            ...
            icorr_sub = pd.merge(icorr_sub, correls[replicate], left_index = True, right_index = True, how = 'outer')
        
        icorr[condition] = icorr_sub
        icorr[condition].fillna(0., inplace = True)
        icorr_mean[condition] = pd.DataFrame()
        icorr_mean[condition]['InnerCorrelation_' + condition] = icorr[condition].mean(axis = 1)  
            ...
            for replicate in data[condition]:
                for sample in data[condition][replicate]:
                    if sample[:sample.find('_')] == prefix:
                        fract_vals = pd.merge(fract_vals, data[condition][replicate][sample], left_index = True, right_index = True, how = 'outer')
                        fract_std = pd.merge(fract_std, data[condition][replicate][sample], left_index = True, right_index = True, how = 'outer')
            cols = [col for col in fract_vals.columns]
            fract_vals[condition + '_median_' + prefix] = fract_vals[cols].median(axis = 1)
            fract_std[condition + '_std_' + prefix] = fract_std[cols].std(axis = 1)
            con_vals = pd.merge(con_vals, fract_vals[condition + '_median_' + prefix], left_index = True, right_index = True, how = 'outer').fillna(0.)
            con_std = pd.merge(con_std, fract_std[condition + '_std_' + prefix], left_index = True, right_index = True, how = 'outer').fillna(0.)
...
def combine_concat (data, window_FDP):
    for condition in data:
        ...
        for replicate in data[condition]:
            ...
            for sample in data[condition][replicate]:
                oldname = sample
                newname = condition + sample[sample.rfind('_'):] + '_' + sample[:sample.find('_')]
                renamedict[oldname] = newname
            con_vals = pd.merge(con_vals, data[condition][replicate], left_index = True, right_index = True, how = 'outer')
            con_vals.rename(renamedict, axis = 'columns', inplace = True)
        data[condition] = {'concat' : con_vals.fillna(0.)}
    return data
...
def remove_zeros (data, window_FDP, progress):
    for condition in data:
        ...
        for replicate in data[condition]:
            ...
            window_FDP['--status2--'].Update(' '.join([condition, replicate]))
            event, values = window_FDP.read(timeout = 50)
            data[condition][replicate] = data[condition][replicate].apply(pd.to_numeric, errors = 'coerce')
            data[condition][replicate] = data[condition][replicate].fillna(0)[~(data[condition][replicate] == 0).all(axis=1)]
    return data, progress
    ...
    for condition in data:
        ...
        for con in data:
            if not con == condition:
                ...
                data_own = data[condition][comb].fillna(0.)
                data_other = data[con][comb].fillna(0.)
                fracts_own = []
                fracts_other = []
        ...
        outer_corrs = pd.merge(outer_corrs, outcorr, left_index = True, right_index = True, how = 'outer')
...
def FDP_exec (window, input_tables, preparams, identifiers, data_ways, std_ways, intermediate_data, protein_info, conditions_trans, fract_indata):
    sg.theme('DarkTeal11')
    layout_FDP = [
        [sg.Column([
            [sg.ProgressBar(100, orientation = 'h', size = (38,25), key = '--progress--')],
            [sg.Text('-ready-', font = ('Arial', 9), size = (60,2), key = '--status1--')],
            [sg.Text('for run', font = ('Arial', 9), size = (60,2), key = '--status2--')],
            ], size = (420,100)),
        sg.Column([
            [sg.Button('Start', size = (15,1), key = '--start--', disabled = False, enable_events = True, button_color = 'dark green')],
            [sg.Button('Cancel', size = (15,1), key = '--cancel--', disabled = False, enable_events = True, button_color = 'black')]
            ], size = (150,70), vertical_alignment='top')]
              ]
    window_FDP = sg.Window('Processing...', layout_FDP, size = (600,120))
    while True:
        ...
        if event_FDP == '--start--':
            ...
            if not is_ident:
                ...
                messagebox.showerror("Error", "Not enough replicates! Load more replicates or reduce threshold in Parameters.")
            else:
                ...
                dataset, protein_info, progress = create_dataset(fract_indata, input_tables, identifiers, conditions, window_FDP, progress)
                ...
                for way in data_ways:
                    data_ways[way], progress = remove_zeros(data_ways[way], window_FDP, progress)
                ...
                for way in data_ways:
                    if preparams[way]['scale1'][0]:
                        data_ways[way], progress = pre_post_scaling(data_ways[way], preparams[way]['scale1'][1], window_FDP, progress)
                intermediate_data['[2] class_prescaled'] = copy.deepcopy(data_ways['class'])
                ...
                if preparams['global']['missing'][0]:
                    for way in data_ways:
                        data_ways[way], progress = filter_missing(data_ways[way], int(preparams['global']['missing'][1]), window_FDP, progress)
                intermediate_data['[3] class_f_missing'] = copy.deepcopy(data_ways['class'])
                ...
                for way in data_ways:
                    data_ways[way], proteins_remaining, progress = filter_count(data_ways[way], int(preparams['global']['minrep'][1]), window_FDP, progress)
                ...
                fracts_con, fracts_count, fracts_corr, progress = list_samples(data_ways['class'], window_FDP, progress)
                ...
                for way in data_ways:
                    data_combined, std_ways[way], progress = combine_median_std(data_ways[way], fracts_con, window_FDP, progress)
                    if preparams[way]['combination'] == 'median':
                        data_ways[way] = data_combined
                        if way == 'class':
                            intermediate_data['[6] class_combined'] = copy.deepcopy(data_ways[way])
                        elif way == 'vis':
                            intermediate_data['[6] vis_combined'] = copy.deepcopy(data_ways[way])
                ...
                for way in data_ways:
                    if preparams[way]['scale2'][0]:
                        data_ways[way], progress = pre_post_scaling(data_ways[way], preparams[way]['scale2'][1], window_FDP, progress)
                intermediate_data['[7] class_postscaled'] = copy.deepcopy(data_ways['class'])
                ...
                progress = 90
                window_FDP['--status1--'].Update(value = 'calculating outer correlations...')
                window_FDP['--progress--'].Update(progress)
                event, values = window_FDP.read(timeout = 50)
                
                if preparams['global']['outcorr']:
                    outcorr, progress = calculate_outcorr(data_ways['vis'], proteins_remaining, preparams['vis']['combination'], window_FDP, progress)
