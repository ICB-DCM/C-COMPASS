...
def RP_gradient_heatmap(fract_data):
    ...
    layout = [
        ...
        [sg.Combo(conditions, key='-CONDITION-', enable_events=True, readonly=True, default_value=None)],
        [sg.Image(key='-HEATMAP-')],
        [sg.Button('Export')]
    ]

    # Create the window with a static size
    window = sg.Window('Hierarchical Clustering Heatmap', layout, size=(600, 550), finalize=True)

    # Custom colormap: from #f2f2f2 (for value 0) to #6d6e71 (for value 1)
    cmap = mcolors.LinearSegmentedColormap.from_list('custom_gray', ['#f2f2f2', '#6d6e71'], N=256)
    ...
    def plot_heatmap(dataframe, condition_name, save_as_pdf=False, folder_path=None):
        ...
        sns.heatmap(df_clustered, cmap=cmap, cbar=True, xticklabels=False, yticklabels=False, vmin=0, vmax=1)
    ...
    def export_results(fract_data, folder_path):
        ...
        with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
            ...
            for condition in fract_data['vis']:
                ...
                plot_heatmap(df, condition_name=condition, save_as_pdf=True, folder_path=folder_path)
...
def RP_stats_heatmap(results):
    ...
    def plot_heatmap(dataframe, condition_name, save_as_pdf=False, folder_path=None):
        ...
        # Plot the heatmap using seaborn with the custom color gradient
        plt.figure(figsize=(8, 6))
        sns.heatmap(df_clustered, cmap=cmap, cbar=True, xticklabels=x_labels, yticklabels=False, vmin=0, vmax=1)
...
def RP_stats_distribution(results):
    ...
    # Create the window with a static size
    window = sg.Window('Class Distribution Pie Chart', layout, size=(600, 550), finalize=True)
    ...
    def export_pie_charts(results, folder_path):
        ...
        with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
            ...
            for condition in results:
                ...
                plot_pie_chart(df, condition_name=condition, save_as_pdf=True, folder_path=folder_path)
...
def RP_global_heatmap(comparison):
    ...
    layout = [
        ...
        [sg.Combo(comparisons, key='-COMPARISON-', enable_events=True, readonly=True, default_value=None)],
        [sg.Image(key='-HEATMAP-')],
        [sg.Button('Export')]
    ]

    # Create the window with a static size
    window = sg.Window('Global Heatmap (Comparisons)', layout, size=(600, 550), finalize=True)

    # Custom colormap: from #730000 (for -1) to #f1f2f2 (for 0) to #1a0099 (for 1)
    cmap = mcolors.LinearSegmentedColormap.from_list('custom_gradient', ['#730000', '#f1f2f2', '#1a0099'], N=256)
    ...
    def plot_heatmap(dataframe, comparison_name, save_as_pdf=False, folder_path=None):
        ...
        # Plot the heatmap using seaborn with the custom color gradient
        plt.figure(figsize=(8, 6))
        sns.heatmap(df_clustered, cmap=cmap, cbar=True, xticklabels=x_labels, yticklabels=False, vmin=-1, vmax=1)
    ...
    def export_heatmaps(comparison, folder_path):
        ...
        with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
            ...
            for comp in comparison:
                ...
                plot_heatmap(df_filtered, comparison_name=str(comp), save_as_pdf=True, folder_path=folder_path)
...
def RP_global_distance(comparison):
    ...
    # Create the window with a static size
    window = sg.Window('Global Distance Scatter Plot (Comparisons)', layout, size=(600, 550), finalize=True)
    ...
    def plot_scatter(dataframe, comparison_name, save_as_pdf=False, folder_path=None):
        ...
        # If we need to save the plot as a PDF file
        if save_as_pdf and folder_path:
            pdf_filename = os.path.join(folder_path, f'{comparison_name}_scatterplot.pdf')
            plt.savefig(pdf_filename, format='pdf')
    ...
    def export_scatter_data(comparison, folder_path):
        ...
        with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
            ...
            for comp in comparison:
                ...
                plot_scatter(df_filtered, comparison_name=str(comp), save_as_pdf=True, folder_path=folder_path)
...
def RP_class_heatmap(results):
    ...
    conditions = list(results.keys())
    classnames_lists = [set(results[condition]['classnames']) for condition in conditions]
    ...
    layout = [
        ...
        [sg.Combo(common_classnames, key='-CLASSNAME-', enable_events=True, readonly=True, default_value=None)],
        [sg.Image(key='-HEATMAP-')],
        [sg.Button('Export')]
    ]

    # Create the window with a static size
    window = sg.Window('Class Clustering Heatmap', layout, size=(600, 550), finalize=True)
    ...
    # Function to plot heatmap and return as a PIL image
    def plot_heatmap(dataframe, classname, conditions, save_as_pdf=False, folder_path=None):
        ...
        try:
            ...
            plt.figure(figsize=(8, 6))
            sns.heatmap(df_clustered, cmap='viridis', cbar=True, xticklabels=conditions, yticklabels=False)
    ...
    def export_heatmaps(results, common_classnames, folder_path):
        ...
        with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
            ...
            for classname in common_classnames:
                ...
                plot_heatmap(df_numeric, classname, conditions, save_as_pdf=True, folder_path=folder_path)
